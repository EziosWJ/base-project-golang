...
VALUES
(1, '启用', '1', 1, NULL), (1, '禁用', '0', 2, NULL),
(2, '未知', 'UNSPECIFIED', 1, NULL), (2, '男', 'MALE', 2, NULL), (2, '女', 'FEMALE', 3, NULL),
(3, '目录', 'DIR', 1, NULL), (3, '菜单', 'MENU', 2, NULL), (3, '外链', 'LINK', 3, NULL),
(4, '启用', '1', 1, NULL), (4, '禁用', '0', 2, NULL),
(5, '新增', 'CREATE', 1, NULL), (5, '修改', 'UPDATE', 2, NULL), (5, '删除', 'DELETE', 3, NULL), (5, '导入', 'IMPORT', 4, NULL), (5, '导出', 'EXPORT', 5, NULL),
(6, '成功', 'SUCCESS', 1, NULL), (6, '失败', 'FAIL', 2, NULL),
(7, '头像', 'avatar', 1, NULL), (7, '导入', 'import', 2, NULL), (7, '附件', 'attachment', 3, NULL),
(8, '系统配置', 'SYSTEM', 1, NULL), (8, '自定义配置', 'CUSTOM', 2, NULL),
(9, '文本', 'TEXT', 1, NULL), (9, '数字', 'NUMBER', 2, NULL), (9, '布尔', 'BOOLEAN', 3, NULL);

INSERT INTO sys_config (id, config_name, config_key, config_value, config_type, value_type, status, is_builtin, remark)
VALUES (1, '日志清空开关', 'system.log-clear-enabled', 'true', 'SYSTEM', 'BOOLEAN', 1, 1, '控制日志清空接口是否可用，dev 默认开启，prod 默认关闭');

SELECT setval(pg_get_serial_sequence('sys_dept', 'id'), (SELECT MAX(id) FROM sys_dept));
...
SELECT setval(pg_get_serial_sequence('sys_dict_type', 'id'), (SELECT MAX(id) FROM sys_dict_type));
