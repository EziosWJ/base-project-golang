...
func TestBuildRegistersSystemRoutes(t *testing.T) {
	router, err := Build(testConfig("test", false), readyProbe{}, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestBuildRegistersSwaggerOnlyInDev(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			router, err := Build(testConfig(test.environment, test.enabled), readyProbe{}, nil, nil, nil, nil, nil, nil, nil)
			if err != nil {
				t.Fatalf("Build() error = %v", err)
			}
