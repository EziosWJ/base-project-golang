...
- **当前后端事实**: `base-api/` 是 Java 行为参考；`base-go-api/` 已有可运行的 Go 平台（Gin、配置、PostgreSQL 连接池、Goose、统一响应、CORS、可观测性与 Swagger），并已迁移 `/api/auth` 的登录、登出、当前用户和当前菜单，以及 `/api/system/role`、`/api/system/menu` 的管理接口。角色和菜单的写操作成功后写入 PostgreSQL 操作审计日志；用户、部门、字典、配置、文件及日志查询接口仍按 Issue 依赖顺序替代 Java 行为。
